<?php
    ...
    if(isset($_REQUEST['collection_id']) || isset($_REQUEST['id'])){
        ...
        require_once '../../../vendor/mercadopago/autoload.php';
          ...
          $curl = curl_init();
           curl_setopt_array($curl, array(
             ...
             CURLOPT_HTTPHEADER => array(
                   'Authorization: Bearer '.$extra['live_access_token']
             ),
           ));
        ...
        if($payments){
            ...
            // pending | approved
            if($status == 'approved' && $payments['payment_status'] == 1){
                ...
                $update = $update->execute(['balance' => $payments['balance'], 'status' => 3, 'delivery' => 2, 'extra' => $ref, 'id' => $payments['payment_id']]);
