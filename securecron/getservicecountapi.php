...
$m = json_decode($InfoAccount, true)['graphql']['user']['edge_followed_by']['count'];
return  intval($m);
}
*/
function get_instagram_followers ($url) {
$username = preg_split('#\/#', rtrim(urldecode($url), '/'));
$username = $username[count($username) - 1];	
$raw = file_get_contents('https://www.instagram.com/web/search/topsearch/?query=' . $username); //replace with user
$data = json_decode($raw, true);
$m1 = $data["users"][0]["user"]["pk"];
$raw = file_get_contents('https://www.instagram.com/graphql/query/?query_id=17851374694183129&id='.$m1.'&first=1'); //replace with user
$data = json_decode($raw, true);
$m = $data["data"]["user"]["edge_followed_by"]["count"];
return  intval($m);
}
...
$username = $username[count($username) - 1];
//$login_cookie = Logincookie('kenanorge25756ewl');
$I = Getcount($username, $login_cookie);
return intval($I);
...
$username = preg_split('#\/#', rtrim(urldecode($url), '/'));
$username = $username[count($username) - 1];
$raw = file_get_contents('https://www.instagram.com/graphql/query/?query_id=17864450716183058&shortcode='.$username.'&first=0'); //replace with user
preg_match('/\"edge_liked_by\"\:\s?\{\"count\"\:\s?([0-9]+)/',$raw,$m);
return intval($m[1]);
}
...
function get_instagram_mention($url) {
$username = preg_split('#\/#', rtrim(urldecode($url), '/'));
$username = $username[count($username) - 1];
$client_id  = "014b3b67eb7b4c25ad596b86a953a7fb";
$url = "https://api.instagram.com/v1/media/".$this->get_instagrm_media_id($url)."?&client_id=". $client_id ;
$data = json_decode(file_get_contents($url), true);
return $data['data']['mention']['count'];
return '0';
}
}
...
$settings = array(
'consumer_key' =>'5Lww6dV3d36g6AWBIvw453ETR',
'consumer_secret' =>'OJVOfSBKTnL11zZ0MEVBFOtkFrzV9lOg3OwTZlDsk29MC2gXMp',
'oauth_access_token' =>'2806733523-kEDzXMC0CB547WHl6Wmv3SPW7Obv9ai6NEXm6A4',
'oauth_access_token_secret' =>'N5qOekGt29SivnOkCo0B6Pe2qxXlRmEJ4SMJid4HZEUiF',
);
$username = preg_split('#\/#', rtrim(urldecode($url), '/'));
$username = $username[count($username) - 1];
...
require_once('twitter/TwitterAPIExchange.php'); //get it from https://github.com/J7mbo/twitter-api-php
$settings = array(
'consumer_key' =>'PlJ3PEkQlTt7RFdCTqacdQ1yQ',
'consumer_secret' =>'YQsfIEw71bNYdwp0vDuJhvYWDM7cmXJoLVssYKdhrWj71ElnDq',
'oauth_access_token' =>'2806733523-2nDoodnYiDCF9PWFJz78DiY98LSGEf9OWpRcKxc',
'oauth_access_token_secret' =>'MWlY9nVenQuujHfEE1hEIw8DJfvc8SopmHLKXQRUZfs61',
); 
$username = preg_split('#\/#', rtrim(urldecode($url), '/'));
$username = $username[count($username) - 1];
$url ='https://api.twitter.com/1.1/statuses/show/'.$username.'.json';
...
$username=@split("/",$url);
$username = $username[count($username) - 1];
$json = file_get_contents("https://graph.facebook.com/".$username."?access_token=1720965928134541|6a7a6134c3a15af5797c53d8a9ef76d5&fields=likes");
$counts = json_decode($json, true);
return $counts['likes'];
return '0';
}
