...
$InfoAccount = InfoAccount($username, $login_cookie);
    $post_id="".$InfoAccount['first_post']['shortcode']."";
      $lastupdate=$InfoAccount['first_post']['taken_at_timestamp'];
...
$post_id="".$InfoAccount['first_post']['shortcode']."";
$lastupdate=$InfoAccount['first_post']['taken_at_timestamp'];
$isvideo=json_decode($InfoAccount, true)['first_post']['is_video'];
if($isvideo=="true"){
return array('http://instagram.com/p/'.$post_id,$lastupdate,$isvideo);
}else{
return 0;
}
}
...
$login_cookie = Logincookie('kenanorge25756ewl');
$InfoAccount = InfoAccount($username, $login_cookie);
$private = json_decode($InfoAccount , TRUE);
$isprivate=$private['graphql']['user']['is_private'];
/*
$insta_source =@file_get_contents('http://api.instagram.com/oembed?url=' . $username);
$data = json_decode($insta_source, true);
$m1 = $data['author_id'];
