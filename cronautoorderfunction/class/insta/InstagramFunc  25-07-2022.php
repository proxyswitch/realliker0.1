...
define('API1', 'https://i.instagram.com');
...
// First Media
// $cookie return Data cookie from Login();
function Firstmedia($username, $cookie)
{
    $Firstmedia = curl(
         API1.'/api/v1/users/web_profile_info/?username='.$username.'',
        'GET',
        ...
        json_decode($Firstmedia[2], true)['data']['user']['edge_owner_to_timeline_media']['edges'][0]['node']['shortcode'];
}
...
function followc($username, $cookie)
{
  ...
  //  instagram->graphql->user->edge_followed_by->count;
    return array(
        "Count" => json_decode($Info[2], true)['data']['user']['edge_followed_by']['count']
    );
}
...
function Userid($username, $cookie)
{
  ...
  //  instagram->graphql->user->edge_followed_by->count;
        //"Userid" => 
            return array(
        json_decode($Userid[2], true)['data']['user']['id']
    );
...
function Getcount($shortcode, $cookie)
{
    ...
    return json_decode($Getcount[2], true)['graphql']['shortcode_media']['edge_media_preview_like']['count'];
}
