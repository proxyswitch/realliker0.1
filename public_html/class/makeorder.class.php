...
function storeorder($case,$id,$orderid,$type,$price,$orginalprice,$beforebalance,$afterbalance,$ipaddress,$service,$date,$saleprice,$sellprice,$peritem,$reqcounts,$startcount,$url,$extdata,$ccomments){
global $dbh;
...
$sql12=$dbh->prepare("insert into smme_users_order_urls(`orderid`,`smmeid`,`url`,`startcount`,`finishcount`,`extdata`,`icomments`)values(?,?,?,?,?,?,?)");
if($sql12->execute(array($neworderid,$id,$url,$startcount,$startcount,$extdata,$ccomments))){
...
function createorders($serviceprovi,$case,$url,$reqcounts,$sellprice,$buyprice,$totalprice,$saleprice,$peritem,$display,$userbalance,$userid,$usergroup,$extdata,$ccomments){
$orginalurl=$url;
global $dbh;
...
$return=$this->storeorder($case,$userid,$order_id,$display,$price,$saleprice,$userbalance,$afterbalance,$_SERVER['REMOTE_ADDR'],$service,date("Y-m-d"),$saleprice,$sellprice,$peritem,$reqcounts,$scounts,$orginalurl,$extdata,$ccomments);	
return $return;	
}
...
function order($postservice,$posturl,$postcount,$extdata,$ccomments){
...
if($balance==true){
$status=$this->createorders($res['site'],$res['ser'],$posturl,$postcount,$res['sellprice'],$res['buyprice'],$res['totalprice'],$res['saleprice'],$res['per_item'],$res['displayid'],$profile['balance'],$profile['smmeid'],$profile['groups'],$extdata,$ccomments);
if($status==true){
return 1;
}else{
return 2;
